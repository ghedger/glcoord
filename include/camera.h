	...
	private:
		...
		void updateCameraVectors()
		{
			...
			// Calculate Phi (Quick hack -- all this needs clean up)
				...
				double xzDelta = sqrt( ( double ) delta.x * ( double ) delta.x + ( double ) delta.z * ( double ) delta.z);
				if( 1 ) {
					//Pitch = asin( xzDelta / delta.y ) * 180 / M_PI;
					CamPitch = asin( ( double )delta.y / (double )xzDelta ) * 180 / M_PI;
					if( std::isnan( CamPitch ) ) {
						CamPitch = -90 + ( asin( xzDelta / delta.y ) * 180 / M_PI );
